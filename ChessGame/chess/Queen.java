...
public class Queen extends Piece {
    ...
    public boolean moveValid(String destination, Board board){
        ...
        if (start_array[1] == destination_array[1] || destination_array[0] == start_array[0]){
            if (clearStraightPath(destination, board)) valid = true; // if move is in straight line and the path is clear, make valid true;
        } 

        if (isDiagonal(start_array, destination_array)){
            if (clearDiagonalPath(destination, board)) valid = true; // check to see if move is diagonal and if path is clear, if so make valid true
        } 
    ...
    public Boolean clearStraightPath(String destination, Board board){
        int[] start_array = Piece.coordinateToInteger(position);
        int[] destination_array = Piece.coordinateToInteger(destination);
    ...
    public boolean clearDiagonalPath(String destination, Board board){
        int[] start_array = Piece.coordinateToInteger(position);
        int[] destination_array = Piece.coordinateToInteger(destination);
